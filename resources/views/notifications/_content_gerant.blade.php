{{--
    resources/views/notifications/_content.blade.php

    Contenu pur de la page notifications (sans layout).
    Inclus par index.blade.php selon le rôle.
--}}
...
<div class="upg upg-wide">
    ...
    <div class="upg-hd">
        <div>
            <div class="upg-title">Notifications</div>
            <div class="upg-sub">Toutes vos alertes et mises à jour.</div>
        </div>
        @if(Auth::user()->unreadNotifications->count() > 0)
        <form method="POST" action="{{ route('notifications.markAllAsRead') }}">
            @csrf
            <button type="submit" class="ubtn ubtn-secondary">
    ...
    <div class="ucard">
        ...
        <div style="display:flex;align-items:flex-start;gap:1rem;padding:1rem 1.25rem;border-bottom:1px solid #e4e8f0;background:{{ $isRead ? '#fff' : $couleur['bg'].'40' }};transition:background .15s"
                ...
                <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;flex-wrap:wrap">
                    <div>
                        ...
                        <div style="font-size:.78rem;color:#4b5563;margin-bottom:.3rem">
                            <span style="font-family:monospace;font-weight:600">{{ $data['declaration_reference'] ?? '' }}</span>
            ...
            <div style="display:flex;align-items:center;gap:.4rem;flex-shrink:0">
                ...
                <a href="{{ route('notifications.markAsRead', $notif->id) }}"
                   style="width:28px;height:28px;border-radius:6px;background:#ecfdf5;border:1px solid #a7f3d0;display:flex;align-items:center;justify-content:center;text-decoration:none;color:#059669;transition:all .15s"
                ...
                <form method="POST" action="{{ route('notifications.destroy', $notif->id) }}">
                    @csrf @method('DELETE')
                    <button type="submit"
                            style="width:28px;height:28px;border-radius:6px;background:#fef2f2;border:1px solid #fecaca;display:flex;align-items:center;justify-content:center;cursor:pointer;color:#dc2626;transition:all .15s"
                            title="Supprimer">
                        <svg style="width:12px;height:12px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg>
                    </button>
                </form>
            </div>
        </div>
        ...
        <div style="text-align:center;padding:4rem 1rem;color:#9ca3af">
            <svg style="width:40px;height:40px;opacity:.2;margin:0 auto .75rem;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
